...
def run_standard_rrt(step_size=1.0, max_iters=500, goal_threshold=0.5, benchmark_mode=False,
                    width=10, height=10, start=(1, 1), goal=(9, 9), obstacles=None, plot_dir=None, plot_number=None):
    """
    Run the standard RRT algorithm with the given parameters.
    ...
    for i in range(max_iters):
        ...
        if not is_in_collision(x_nearest, x_new, obstacles):
            nodes.append(x_new)
            parents[x_new] = x_nearest
...
if __name__ == "__main__":
    ...
    if benchmark_mode:
        result_path = os.environ.get("RESULT_PATH", "results/result.json")
