...
def run_rrt_star_variant(step_size=1.0, max_iters=1000, goal_threshold=0.5, gamma=2.5, 
                         alpha=1.0, beta=1.0, dimension=2, width=10, height=10, 
                         start=(1, 1), goal=(9, 9), obstacles=None, benchmark_mode=False,
                         plot_dir=None, plot_number=None):
    """
    Run the RRT* variant algorithm with the given parameters.
    ...
    Returns:
        Dictionary with results of the RRT* variant run
    """
    ...
    for i in range(max_iters):
        ...
        for node in near:
            if not is_in_collision(node, x_new, obstacles):
                cost = costs[node] + alpha * distance(node, x_new) + beta * distance(x_rand, goal)
                if cost < min_cost:
                    min_cost = cost
                    min_parent = node
        ...
        for node in near:
            if node == min_parent:
                continue
            if not is_in_collision(x_new, node, obstacles):
                new_cost = costs[x_new] + distance(x_new, node)
    ...
    if not benchmark_mode and goal_reached:
        fig, ax = plt.subplots()
        ax.set_xlim(0, width)
