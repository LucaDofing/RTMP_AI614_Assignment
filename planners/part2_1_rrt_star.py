...
def run_rrt_star(step_size=1.0, max_iters=1000, goal_threshold=0.5, gamma=2.5, benchmark_mode=False,
                width=10, height=10, start=(1, 1), goal=(9, 9), obstacles=None, dimension=2, plot_dir=None, plot_number=None):
    """
    Run the RRT* algorithm with the given parameters.
    
    Args:
        step_size: The step size for extending the tree
        max_iters: Maximum number of iterations
        goal_threshold: Distance threshold to consider goal reached
        gamma: Parameter for determining radius of neighborhood search
    ...
    for i in range(max_iters):
        ...
        min_cost = costs[x_nearest] + distance(x_nearest, x_new)

        for node in near:
            if not is_in_collision(node, x_new, obstacles):
                cost = costs[node] + distance(node, x_new)
                if cost < min_cost:
                    min_cost = cost
                    min_parent = node
