...
def run_heuristic_rrt(step_size=1.5, max_iters=300, goal_threshold=0.5, benchmark_mode=False,
                     width=10, height=10, start=(1, 1), goal=(9, 9), obstacles=None, plot_dir=None, plot_number=None):
    """
    Run the heuristic RRT algorithm with the given parameters.
    ...
    def steer(p1, p2, step_size):
        ...
        return tuple(np.array(p1) + step_size * vec / dist) if dist > 0 else p1

    def closest_to_goal(nodes, goal):
        return min(nodes, key=lambda n: np.linalg.norm(np.array(n) - np.array(goal)))
    ...
    for i in range(max_iters):
        x_tree = closest_to_goal(nodes, goal)
        ...
        # print life signal all 1000 iterations
        if i % 1000 == 0 and not benchmark_mode:
            print(f"[INFO] Iteration {i}: Nodes = {len(nodes)}")
...
if __name__ == "__main__":
    default_config_path = os.path.join(os.path.dirname(__file__), "..", "results", "heuristic_config.json")
    ...
    print(f"[INFO] Running heuristic RRT with step_size={step_size}, max_iters={max_iters}")

    result = run_heuristic_rrt(
        step_size=step_size, 
        max_iters=max_iters, 
    ...
    if benchmark_mode:
        fig_name = f"figures/heuristic/heuristic_stepsize{step_size:.1f}_iters{max_iters}_thr{goal_threshold}.png"
